...
const DeedsDashboard: React.FC<DeedsDashboardProps> = ({ currentUserRole, currentUserName, filteredProjectName }) => {
    ...
    const fetchDeeds = async () => {
        ...
        try {
            ...
            if (filteredProjectName) {
                query = query.eq('project_name', filteredProjectName);
            }
    ...
    const handleSaveNewDeed = async () => {
        if (!newDeedForm.clientName || !newDeedForm.idNumber) {
            return alert("يرجى إكمال البيانات الأساسية (الاسم والهوية)");
        }
        setIsSaving(true);
        try {
            const dbData = {
    ...
    const handlePostComment = async (deedId: number) => {
        ...
        try {
            ...
            fetchDeeds(); // Refresh to show new comment
        } catch (e: any) {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                const formatted = data.map(r => ({
                    ...
                    plan_number: r['رقم المخطط'] || '',
                    unit_number: r['رقم الوحدة'] || '',
                    old_deed_number: r['رقم الصك'] || '',
                    old_deed_date: r['تاريخ الصك'] || '',
                    client_name: r['اسم المستفيد'] || '',
                    id_number: String(r['هوية المستفيد'] || ''),
                    mobile: String(r['رقم الجوال'] || ''),
                    dob: r['تاريخ الميلاد'] || '',
                    unit_value: parseFloat(r['قيمة الوحدة'] || '0'),
                    tax_number: r['الرقم الضريبي'] || '',
                    bank_name: r['الجهة التمويلية'] || '',
                    contract_type: r['نوع العقد'] || 'مرابحة',
                    new_deed_number: r['رقم الصك الجديد'] || '',
                    new_deed_date: r['تاريخ الصك الجديد'] || '',
                    sakani_support_number: r['رقم عقد الدعم'] || '',
                    status: 'جديد',
                    submitted_by: currentUserName
        ...
        <div className="space-y-8 animate-in fade-in duration-500 font-cairo text-right" dir="rtl">
                    ...
                    {canManage && (
                        <>
                            <button 
                                ...
                                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-[#1B2B48] text-white rounded-2xl font-black text-sm hover:brightness-110 transition-all active:scale-95 shadow-lg shadow-blue-100"
                            >
                                <FileUp size={18} /> رفع ملف إكسل
                            </button>
                            <button 
                                onClick={() => setIsRegModalOpen(true)} 
                                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-3 bg-[#E95D22] text-white rounded-2xl font-black text-sm hover:brightness-110 shadow-xl shadow-orange-100 transition-all active:scale-95"
                            >
                                        ...
                                        <tr className="hover:bg-gray-50 cursor-pointer group" onClick={() => toggleRow(deed.id)}>
                                            <td className="p-6">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center font-black text-xs">
                                                        {deed.clientName?.[0]}
                                                    </div>
                                                    <div>
                                                        <p className="font-bold text-[#1B2B48] text-sm">{deed.clientName}</p>
                                                        <p className="text-[10px] font-mono text-gray-400 font-bold">{deed.idNumber}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="p-6">
                                                <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black border ${
                                                    deed.status === 'مكتمل' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-blue-50 text-blue-700 border-blue-100'
                                                }`}>
                                                    {deed.status}
                                                </span>
                                            </td>
                                            <td className="p-6 text-gray-300">
                                                {expandedRows.has(deed.id) ? <ChevronUp size={20}/> : <ChevronDown size={20}/>}
                                            </td>
                                        </tr>
                                        
                                        {/* EXPANDED ROW: COMMENTS & TICKET DETAILS */}
                                                    ...
                                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                                        ...
                                                        <div className="bg-white rounded-[30px] border border-gray-100 p-8 shadow-sm flex flex-col h-[400px]">
                                                            <div className="flex justify-between items-center mb-6">
                                                                <h4 className="font-black text-[#1B2B48] flex items-center gap-2">
                                                                    <MessageSquare size={20} className="text-[#E95D22]" /> ملاحظات وتحديثات الطلب
                                                                </h4>
                                                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{deed.comments.length} تعليق</span>
                                                            </div>
                                                            
                                                            <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar mb-6">
                                                                {deed.comments.length === 0 ? (
                                                                    <div className="h-full flex flex-col items-center justify-center opacity-30">
                                                                        <History size={48} className="mb-2" />
                                                                        <p className="text-xs font-bold italic">لا توجد تحديثات سابقة</p>
                                                                    </div>
                                                                ) : (
                                                                    deed.comments.map((c: any) => (
                                                                        <div key={c.id} className={`p-4 rounded-2xl border ${c.author === currentUserName ? 'bg-blue-50 border-blue-100 ml-8' : 'bg-gray-50 border-gray-100 mr-8'}`}>
                                                                            <div className="flex justify-between items-center mb-1">
                                                                                <span className="text-[10px] font-black text-[#1B2B48]">{c.author}</span>
                                                                                <span className="text-[9px] text-gray-400 font-bold" dir="ltr">{new Date(c.created_at).toLocaleTimeString('ar-SA')}</span>
                                                                            </div>
                                                                            <p className="text-xs text-gray-600 font-bold leading-relaxed">{c.text}</p>
                                                                        </div>
                                                                    ))
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className="flex gap-2">
                                                                <input 
                                                                    type="text" 
                                                                    placeholder="أضف ملاحظة أو تحديثاً للحالة..." 
                                                                    className="flex-1 p-4 bg-gray-50 rounded-2xl text-xs font-bold outline-none focus:ring-2 focus:ring-[#E95D22]/10 transition-all border border-transparent focus:border-[#E95D22]/20"
                                                                    value={commentInput[deed.id] || ''}
                                                                    onChange={(e) => setCommentInput({...commentInput, [deed.id]: e.target.value})}
                                                                    onKeyDown={(e) => e.key === 'Enter' && handlePostComment(deed.id)}
                                                                />
                                                                <button 
                                                                    onClick={() => handlePostComment(deed.id)}
                                                                    className="p-4 bg-[#1B2B48] text-white rounded-2xl hover:bg-[#E95D22] transition-all active:scale-95 shadow-lg"
                                                                >
                                                                    <Send size={18} />
                                                                </button>
                                                            </div>
                                                        ...
                                                        <div className="bg-white rounded-[30px] border border-gray-100 p-8 shadow-sm">
                                                            <h4 className="font-black text-[#1B2B48] mb-6 flex items-center gap-2">
                                                                <Info size={20} className="text-[#E95D22]" /> التفاصيل المسجلة
                                                            </h4>
            ...
            {isRegModalOpen && canManage && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#1B2B48]/40 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white rounded-[45px] w-full max-w-7xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh]">
                        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-[#1B2B48]">تسجيل إفراغ جديد</h3>
                                    <p className="text-[10px] text-gray-400 font-black mt-1 uppercase tracking-widest">تعبئة نموذج البيانات الموحد - 18 حقلاً</p>
                                </div>
                            </div>
                        ...
                        <div className="p-8 border-t border-gray-100 bg-white flex justify-end gap-5 shrink-0">
                            <button onClick={() => setIsRegModalOpen(false)} className="px-10 py-4 border border-gray-200 rounded-[22px] font-black text-gray-500 hover:bg-gray-50 text-sm transition-all active:scale-95">إلغاء</button>
                            <button onClick={handleSaveNewDeed} disabled={isSaving} className="px-16 py-4 bg-[#E95D22] text-white rounded-[22px] font-black hover:brightness-110 shadow-2xl shadow-orange-100 text-sm flex items-center gap-2 active:scale-95 transition-all disabled:opacity-50">
                                {isSaving ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle2 size={20}/>} {isSaving ? 'جاري الحفظ...' : 'حفظ الطلب'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {isImportModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#1B2B48]/40 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white rounded-[40px] w-full max-w-lg shadow-2xl overflow-hidden p-8 animate-in zoom-in-95 duration-300 text-center">
                        ...
                        <p className="text-gray-400 font-bold mb-8 text-sm px-10">يرجى اختيار ملف Excel يحتوي على الأعمدة المعتمدة (المنطقة، المستفيد، رقم الصك... إلخ)</p>
    ...
    <div className="space-y-2">
        ...
        <div className="relative group">
            ...
            <input 
                ...
                className={`w-full ${icon ? 'pr-12' : 'pr-4'} pl-4 py-4 bg-gray-50 border border-transparent rounded-[20px] outline-none focus:border-[#E95D22] focus:bg-white focus:ring-4 focus:ring-orange-50 text-sm font-bold transition-all shadow-sm ${disabled ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''}`} 
                value={value || ''} 
...
const FormSelect = ({ label, options, value, onChange }: any) => (
    <div className="space-y-2">
        ...
        <div className="relative">
            <select 
                className="w-full pr-4 pl-10 py-4 bg-gray-50 border border-transparent rounded-[20px] outline-none focus:border-[#E95D22] focus:bg-white focus:ring-4 focus:ring-orange-50 text-sm font-bold appearance-none cursor-pointer transition-all shadow-sm" 
                value={value || ''} 
                onChange={(e) => onChange(e.target.value)}
            >
