                            ...
                            <div className="lg:col-span-1 space-y-10">
                              ...
                              {completedRequestsForCurrentProject.length > 0 && (
                                <div className="space-y-6 animate-in slide-in-from-right duration-500">
                                  <h3 className="text-xl font-bold text-[#1B2B48] flex items-center gap-2"><Briefcase className="text-[#E95D22]" /> الطلبات الحكومية المنجزة</h3>
                                  <div className="space-y-3">
                                    {completedRequestsForCurrentProject.map(req => (
                                      <div 
                                        key={req.id} 
                                        onClick={() => { setSelectedRequestForDetails(req); setIsRequestDetailModalOpen(true); }}
                                        className="bg-gray-50 p-4 rounded-2xl border-2 border-transparent hover:border-green-200 transition-all cursor-pointer flex justify-between items-center group"
                                      >
                                        <div className="flex items-center gap-3">
                                          <div className="p-2 bg-green-100 text-green-600 rounded-lg group-hover:scale-110 transition-transform">
                                            {req.type === 'technical' ? <Settings size={18} /> : <FileCheck size={18} />}
                                          </div>
                                          <div>
                                            <p className="text-sm font-bold text-[#1B2B48]">{req.type === 'conveyance' ? `إفراغ: ${req.clientName}` : req.serviceSubType}</p>
                                            <p className="text-[10px] text-gray-400">{req.date}</p>
                                          </div>
                                        </div>
                                        <ChevronLeft size={16} className="text-gray-300 group-hover:text-green-500 transition-colors" />
                                      </div>
                                    ))}
                                  </div>
                                </div>
